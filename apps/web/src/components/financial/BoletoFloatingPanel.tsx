...
interface BoletoFloatingPanelProps {
  ...
  tenantName?: string;
  children: React.ReactNode;          // trigger element (botão no grid)
  className?: string;
}
...
// Gera barras visuais CSS para o código de barras
...
// Gera o HTML completo do boleto para exibição em iframe
...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .h-bank{flex:0 0 200px;padding:10px 12px;border-right:2px solid #333;font-size:18px;font-weight:bold;display:flex;align-items:center}
    .h-code{flex:0 0 80px;padding:10px 12px;border-right:2px solid #333;font-size:16px;font-weight:bold;display:flex;align-items:center;justify-content:center}
    .h-line{flex:1;padding:10px 12px;font-family:monospace;font-size:13px;letter-spacing:2px;font-weight:bold;display:flex;align-items:center;word-break:break-all}
    ...
    .fv-lg{font-size:16px;font-weight:bold}
    .desc{padding:8px 10px;border-bottom:1px solid #ccc}
    .bc{padding:16px 10px 8px;border-bottom:1px solid #ccc;text-align:center}
    .bc-bars{display:flex;align-items:flex-end;justify-content:center;height:60px;margin-bottom:8px}
    .bc-num{font-family:monospace;font-size:11px;letter-spacing:1px;color:#333}
    .inst{padding:10px;background:#f5f5f5;font-size:9px;color:#555;line-height:1.6}
...
<body>
  ...
  <script>
    // Permite que o parent chame print() via postMessage
...
export function BoletoFloatingPanel({
  ...
  const openPanel = useCallback(() => {
    if (triggerRef.current) {
      ...
      let x = spaceRight >= PANEL_W + 12 ? rect.right + 10 : rect.left - PANEL_W - 10;
      let y = Math.max(10, Math.min(rect.top - 20, window.innerHeight - PANEL_H - 10));
      // Se sair da tela à esquerda, centraliza
      if (x < 10) x = Math.max(10, (window.innerWidth - PANEL_W) / 2);
      setPosition({ x, y });
    }

    if (!blobUrl) {
      const html = buildBoletoHtml(charge, tenantName);
      const blob = new Blob([html], { type: 'text/html' });
      setBlobUrl(URL.createObjectURL(blob));
    }

    setIsOpen(true);
  }, [blobUrl, charge, tenantName]);

  // Revogar blob ao desmontar
  useEffect(() => {
  ...
  const handleDownload = () => {
    ...
    a.href = blobUrl;
    const safeName = description.replace(/[^a-z0-9\-_]/gi, '_').slice(0, 40);
    a.download = `boleto_${safeName}.html`;
    a.click();
    ...
    <>
      ...
      {isOpen && (
        <div
          ...
          style={{ top: position.y, left: position.x, width: 640, height: 720 }}
        >
          {/* ── Header ── */}
          <div
          ...
          >
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className="truncate text-xs font-medium text-slate-300 max-w-[260px]">
                {description}
              </span>
              {isMock && (
            ...
            <div className="flex shrink-0 items-center gap-1.5">
              ...
              <button
                ...
                className="flex items-center gap-1 rounded border border-emerald-500/40 bg-emerald-500/20 px-2 py-1 text-[10px] font-semibold text-emerald-300 transition hover:bg-emerald-500/40 hover:text-white"
                title="Salvar boleto"
              >
                <Download size={11} />
                Salvar
